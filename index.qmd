...
::: {.callout-note title="Herleitung - Der Unterschied zur inneren Energie<br><small>[Hey! Klapp das hier nicht auf, wenn Du geistig gesund bleiben willst.]{fg='ivory-shade-4'}</small>" collapse="true"}
...
$$
\Delta \left( pV \right) = \left( pV \right)\sub{Ende} - \left( pV \right)\sub{Anfang} = p\sub{Ende} V\sub{Ende} - p\sub{Anfang} V\sub{Anfang}.
$$
...
::: {.callout-note title="Herleitung - Die Größe der Enthalpie<br><small>[Nicht schon wieder Buchstaben! Das ist die zweite Warnung. Klapp das bloß nicht auf!]{fg='ivory-shade-4'}</small>" collapse="true"}
...
- $\Delta H > 0$: Wenn die Enthalpieänderung größer als Null ist, dann handelt es sich um eine **endotherme Reaktion**. Das Pluszeichen sagt uns, dass das System Energie dazu gewinnt. Das passiert, weil die Umgebung Wärme an das System abgibt. In der Umgebung einer endothermen Reaktion wird es kalt.
